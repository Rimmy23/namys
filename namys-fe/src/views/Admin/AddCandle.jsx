...
function AddCandle() {
  ...
  const [candle, setCandle] = useState({
    ...
    parameterCandle: "",
    image_candle_main: null,
    image_candle_first: null,
    image_candle_second: null,
    image_url0: null,
    image_url1: null,
    image_url2: null,
    priceCandle: "",
  ...
  const onImageChoose0 = (ev) => {
    ...
    reader.onload = () => {
      setCandle({...candle, image_candle_main: file, image_url0: reader.result,});
      ev.target.value="";
    };
  ...
  const onImageChoose1 = (ev) => {
    ...
    const reader = new FileReader();
    reader.onload = () => {
      setCandle({...candle, image_candle_first: file, image_url1: reader.result,});
      ev.target.value="";
    };
    reader.readAsDataURL(file);
  };
  ...
  const onImageChoose2 = (ev) => {
    ...
    const reader = new FileReader();
    reader.onload = () => {
      setCandle({...candle, image_candle_second: file, image_url2: reader.result,});
      ev.target.value="";
    };
    reader.readAsDataURL(file);
  };
  ...
  const onSubmit = (ev) =>{
    ...
    if (payload.image_candle_main){
      payload.image_candle_main=payload.image_url0;
    }
    delete payload.image_url0;

    if (payload.image_candle_first){
      payload.image_candle_first=payload.image_url1;
    }
    delete payload.image_url1;
    
    if (payload.image_candle_second){
      payload.image_candle_second=payload.image_url2;
    }
    delete payload.image_url2;

    axiosClient.post("/candle", payload).then((res)=>{
      console.log(res);
        ...
        <div className="max-w-sm grid grid-cols-1 items-center md:max-w-4xl  lg:max-w-7xl lg:gap-x-8 lg:px-0 xl:grid-cols-2 xl:h-full">
          <div className="aspect-h-2 aspect-w-3 overflow-hidden rounded-lg border-2 border-gray-500 w-full h-full flex items-center justify-center">  
            <div className="grid grid-row-2">
              {candle.image_url0 && (<img src={candle.image_url0} alt="" className="w-full h-full object-cover"/>)}
              {!candle.image_url0 && (
              <>
                <label htmlFor="image_candle_main" className="cursor-pointer relative flex items-center justify-center">
              ...
              <input 
                ...
                id="image_candle_main" 
                name="image_candle_main" 
                type="file"
          ...
          <div className="aspect-h-2 aspect-w-3 overflow-hidden rounded-lg border-2 border-gray-500 w-full h-full flex items-center justify-center">  
            <div className="grid grid-row-2">
              {candle.image_url1 && (<img src={candle.image_url1} alt="" className="w-full h-full object-cover"/>)}
              {!candle.image_url1 && (
                <>
                  <label htmlFor="image_candle_first" className="cursor-pointer relative flex items-center justify-center">
              ...
              <input 
                ...
                aria-describedby="file_input_help" 
                id="image_candle_first" 
                name="image_candle_first" 
                type="file" 
          ...
          <div className="aspect-h-2 aspect-w-3 overflow-hidden rounded-lg border-2 border-gray-500 w-full h-full flex items-center justify-center">  
            <div className="grid grid-row-2">
              {candle.image_url2 && (<img src={candle.image_url2} alt="" className="w-full h-full object-cover"/>)}
              {!candle.image_url2 && (
                <>
                  <label htmlFor="image_candle_second" className="cursor-pointer relative flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-40 h-40">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
                  </label>
              ...
              <input 
                className="w-40 h-40 cursor-pointer opacity-0 absolute" 
                aria-describedby="file_input_help" 
                id="image_candle_second" 
                name="image_candle_second" 
                type="file" 
        ...
        <div className="mx-auto max-w-2xl px-4 pb-10 pt-10  sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8 lg:px-8 lg:pb-24 lg:pt-0">
          <div className="lg:col-span-2 lg:border-r lg:border-gray-200 lg:pr-8">
            ...
            <input 
              ...
              value={candle.nameCandle} 
              onChange={(ev)=>setCandle({...candle, nameCandle: ev.target.value})} 
          ...
          <div className="py-10 lg:col-span-2 lg:col-start-1 lg:border-r lg:border-gray-200 lg:pb-16 lg:pr-8 lg:pt-6">
            ...
            <div>
              ...
              <textarea 
                id="productDescribeCandle" 
                name="productDescribeCandle" 
                rows="13"
                placeholder="Zde bude popis produktu..." 
                value={candle.productDescribeCandle}
                onChange={(ev)=>setCandle({...candle, productDescribeCandle: ev.target.value})}
            ...
            <div className="mt-10">
              ...
              <div className="mt-4">
                <textarea 
                  id="parameterCandle" 
                  name="parameterCandle" 
                  rows="7"
                  placeholder="Zde jsou parametry produktu..."
                  value={candle.parameterCandle}
                  onChange={(ev)=>setCandle({...candle, parameterCandle: ev.target.value})}
                  className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"></textarea>
              </div>
            </div>
          </div>
            ...
            <div className="mt-4 items-center justify-center lg:row-span-3 lg:-mt-24">
              ...
              <form className="max-w-lg mx-auto mt-4 ml-3 flex items-center justify-center">
                <input 
                  ...
                  placeholder="425" 
                  required
                  value={candle.priceCandle}
                  onChange={(ev)=>setCandle({...candle, priceCandle: ev.target.value})}  
